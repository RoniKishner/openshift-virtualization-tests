...
from tests.infrastructure.golden_images.utils import get_diff_storage_class_from_matrix
...
def fedora_vm_from_data_source(
    ...
    fedora_data_source = DataSource(client=unprivileged_client, name="fedora", namespace=golden_images_namespace.name)
    set_storage_class = request.param.get("set_storage_class")
    storage_class = get_diff_storage_class_from_matrix(data_source=fedora_data_source) if set_storage_class else None
    with VirtualMachineForTests(
        ...
        data_volume_template=data_volume_template_with_source_ref_dict(
            data_source=fedora_data_source, storage_class=storage_class
        ),
    ) as vm:
        yield vm
