...
def available_rwx_storage_class_name(admin_client, available_storage_classes_names):
    for storage_class_name in available_storage_classes_names:
        if (
            StorageProfile(client=admin_client, name=storage_class_name).first_claim_property_set_access_modes()[0]
