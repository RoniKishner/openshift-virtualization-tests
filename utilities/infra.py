...
def assert_os_version_mismatch_in_vm(vm: utilities.virt.VirtualMachineForTests, expected_os: str) -> None:
    ...
    if match:
        expected_os_params = match.groupdict()
    else:
        pytest.fail(f"Did not find matching os_name and os_ver for {expected_os}")
    if expected_os_params["os_name"] == OS_FLAVOR_RHEL:
        expected_os_params["os_name"] = "red hat"
    assert re.match(rf"({expected_os_params['os_name']}).*({expected_os_params['os_ver']}).*", vm_os), (
